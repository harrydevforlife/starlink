...
class BenchmarkSuite:
    ...
    def __init__(self, parquet_file: str, iterations: int = 5):
...
def setup_starlink(parquet_file: str) -> ExecutionContext:
    """Setup Starlink execution context."""
    ctx = ExecutionContext({})
    ctx.register_parquet("tripdata", parquet_file)
    return ctx


def setup_pandas(parquet_file: str):
...
def setup_duckdb(parquet_file: str):
    """Setup DuckDB connection."""
    if not DUCKDB_AVAILABLE:
        return None
    conn = duckdb.connect()
    conn.execute(f"CREATE TABLE tripdata AS SELECT * FROM read_parquet('{parquet_file}')")
    return conn


def benchmark_simple_select(bench: BenchmarkSuite, parquet_file: str):
    """Benchmark: Simple SELECT with projection."""
    
    # Starlink
    def starlink_select():
        ctx = setup_starlink(parquet_file)
        df = ctx.sql("SELECT passenger_count, fare_amount FROM tripdata")
        results = list(ctx.execute_batches(df))
        return sum(batch.row_count() for batch in results)
    
    # Pandas
    def pandas_select():
        df = setup_pandas(parquet_file)
        result = df[["passenger_count", "fare_amount"]]
        return len(result)
    
    # DuckDB
    def duckdb_select():
        conn = setup_duckdb(parquet_file)
...
def benchmark_filter(bench: BenchmarkSuite, parquet_file: str):
...
def benchmark_aggregation(bench: BenchmarkSuite, parquet_file: str):
...
def benchmark_count(bench: BenchmarkSuite, parquet_file: str):
...
def benchmark_complex_query(bench: BenchmarkSuite, parquet_file: str):
...
def main():
    ...
    parser.add_argument(
        "--parquet",
        type=str,
        default="data/yellow_tripdata_2019-01.parquet",
    ...
    parquet_file = Path(__file__).parent / args.parquet
    ...
    bench = BenchmarkSuite(str(parquet_file), iterations=args.iterations)
    
    # Run benchmarks
    benchmark_simple_select(bench, str(parquet_file))
    benchmark_filter(bench, str(parquet_file))
    benchmark_aggregation(bench, str(parquet_file))
    benchmark_count(bench, str(parquet_file))
    benchmark_complex_query(bench, str(parquet_file))
